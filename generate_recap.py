...
Separate rank + logo columns (Presto strips div inside td).
...
TBL="width:100%;border-collapse:collapse;"+FN+";font-size:12px;border:1px solid #ddd;border-top:none"
...
def rk_td(rk,gold=False):
    rc="color:#d4930d;font-weight:800" if gold else "font-weight:700;color:#333"
    return f'<td style="{TD};text-align:center;width:28px;{rc}">{rk}</td>'
def nba_logo_td(url):
    if url and "cdn.nba.com" in url:
        return f'<td style="{TD};width:28px;text-align:center"><img src="{url}" width="22" height="22" style="vertical-align:middle"></td>'
    return f'<td style="{TD};width:28px"></td>'
def flag_logo_td(url):
    if url and "wikimedia" in url:
        return f'<td style="{TD};width:32px;text-align:center"><img src="{url}" width="26" height="18" style="vertical-align:middle;border-radius:2px"></td>'
    return f'<td style="{TD};width:32px"></td>'
...
def build_presto_html(secs,nm):
    ...
    o=f'<div style="{FN};max-width:700px;margin:0 auto;color:#333">'
    ...
    for sn,sr in secs:
        ...
        if sn=="MILESTONES":
            o+=f'<th style="{TH};width:28px"></th><th style="{TH}">PLAYER</th><th style="{TH}">CATEGORY</th><th style="{TH};text-align:right">RANK</th><th style="{TH}">PASSED</th></tr></thead><tbody>'
            for i,row in enumerate(sr):
                ...
                o+=f'<tr style="background:{bg(i)}">{nba_logo_td(lg)}<td style="{TD};font-weight:600;color:#222">{n}</td><td style="{TD};color:#555;font-size:11px">{ct}</td><td style="{TD};font-weight:700;color:#1a5276;text-align:right;font-size:13px">{rk}</td><td style="{TD};color:#555;font-size:11px">{ps}</td></tr>'
        elif sn=="NET RATING":
            o+=f'<th style="{TH};width:28px"></th><th style="{TH};width:32px"></th><th style="{TH}">COUNTRY</th><th style="{TH}">STATS</th><th style="{TH};text-align:center">PLAYERS</th></tr></thead><tbody>'
            rc=0
            for i,row in enumerate(sr):
                ...
                gold=rn!="" and rc<=3
                o+=f'<tr style="background:{bg(i)}">{rk_td(rn,gold)}{flag_logo_td(lg)}<td style="{TD};font-weight:600;color:#222">{n}</td><td style="{TD};color:#555;font-size:11px">{st}</td><td style="{TD};text-align:center;color:#555">{pl}</td></tr>'
        elif sn=="SNEAKERS":
            ...
            o+=f'<th style="{TH};width:28px"></th><th style="{TH};width:32px"></th><th style="{TH}">PLAYER</th><th style="{TH};text-align:right">{rl}</th><th style="{TH}">STATS</th></tr></thead><tbody>'
            for i,row in enumerate(sr):
                ...
                o+=f'<tr style="background:{bg(i)}">{rk_td(i+1,gold)}{flag_logo_td(lg)}<td style="{TD};font-weight:600;color:#222">{n}</td><td style="{TD};font-weight:700;{rtc};text-align:right;font-size:13px">{rat}</td><td style="{TD};color:#555;font-size:11px">{st}</td></tr>'
        else:
            o+=f'<th style="{TH};width:28px"></th><th style="{TH};width:28px"></th><th style="{TH}">PLAYER</th><th style="{TH};text-align:right">{rl}</th><th style="{TH}">STATS</th></tr></thead><tbody>'
            for i,row in enumerate(sr):
                n=hh(row[0].strip());rat=row[1].strip();st=row[2].strip() if len(row)>2 else "";lg=row[11].strip() if len(row)>11 else ""
                gold=i<3;rtc="color:#c0392b" if rat.startswith("-") else "color:#1a5276"
                o+=f'<tr style="background:{bg(i)}">{rk_td(i+1,gold)}{nba_logo_td(lg)}<td style="{TD};font-weight:600;color:#222">{n}</td><td style="{TD};font-weight:700;{rtc};text-align:right;font-size:13px">{rat}</td><td style="{TD};color:#555;font-size:11px">{st}</td></tr>'
        o+='</tbody></table>'
